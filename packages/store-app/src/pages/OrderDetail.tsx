...
export default function OrderDetail() {
  ...
  // Price inputs for each material
  ...
  async function fetchOrder() {
    ...
    if (error || !data) { toast.error('Order not found'); navigate('/dashboard'); return }
  ...
  // Price submission (quote_sent status)
  ...
  async function sendPrices() {
    ...
    if (!confirmed) return
    setSaving(true)
    const updatedMaterials: QuoteMaterial[] = order.quote_materials.map((m, i) => ({
      ...m, price: Number(prices[i])
    }))
    const newTotalQuote = (order.quote_labour || 0) + materialTotal
  ...
  // OTP verification (in_progress status)
  ...
  return (
    <div className="min-h-dvh bg-slate-950 pb-8">
      ...
      <div className="sticky top-0 bg-slate-900 border-b border-slate-800 px-4 py-3 flex items-center gap-3 z-10">
        <button onClick={() => navigate('/dashboard')} className="text-slate-400 p-1">
          <ArrowLeft size={20} />
        </button>
        <div>
      ...
      <div className="px-4 pt-4">
            ...
            {materialTotal > 0 && (
              <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 mb-4">
                ...
                <div className="flex justify-between text-sm text-slate-400 mb-2">
                  <span>GetMyPro ({store?.commission_pct}%)</span>
        ...
        {isInProgress && (
          <>
            ...
            <div className="bg-orange-500/10 border border-orange-500/30 rounded-2xl p-4 mt-4 mb-4 text-center">
              ...
              <p className="text-slate-500 text-xs mt-2">Ask worker for their 6-digit OTP to confirm collection</p>
            </div>
        ...
        {/* Fallback for other statuses (done_uploaded, completed, cancelled, etc.) */}
          ...
          <div className="text-center py-8">
            ...
            {order.store_earnings != null && (
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mt-4">
                <p className="text-slate-400 text-sm">Store Earnings</p>
                <p className="text-green-400 font-black text-2xl">₹{order.store_earnings.toFixed(2)}</p>
              </div>
            )}
          </div>
